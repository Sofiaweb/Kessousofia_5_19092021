// Récupération de l'identifiant du produit contenu dans l'url de la page
...
const itemQuantityber = document.querySelector("#itemQuantity");
...
function getArticles() {
  ...
  fetch(`http://localhost:3000/api/products/${id}`)
    ...
    .then(function (resultatAPI) {
      ...
      pageTitle.innerHTML = article.name;
...
function addToCart() {
  ...
  addToCartBtn.addEventListener("click", () => {
    if (itemQuantityber.value > 0 && itemQuantityber.value < 100) {
      ...
      textConfirmation.innerHTML = `Vous avez ajouté ${itemQuantityber.value} produit(s) à votre panier !`;
